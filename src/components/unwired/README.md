...
| `PermissionSteps.tsx` | The permissions step from onboarding: asks the browser for notifications and location for real, reports exactly what it said, and renders nothing at all for an API the browser lacks. | Removed from the web flow because neither permission pays for itself there — nothing sends a notification yet, and location is asked for at the moment it is needed. Restore it in the native build, where push has somewhere to go. |
